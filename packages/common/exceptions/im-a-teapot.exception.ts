...
/**
 * 定义 *ImATeapotException* 类型错误的 HTTP 异常。
 *
 * 任何试图用茶壶冲咖啡的行为都应该导致错误代码 "418 I'm a teapot"。
 * 生成的实体可能又短又粗。
 *
...
export class ImATeapotException extends HttpException {
   ...
   * - `statusCode`：这将是值 418。
   * - `message`：默认为字符串 `"I'm a Teapot"`；通过在 `objectOrError` 参数中提供字符串来覆盖。
   *
