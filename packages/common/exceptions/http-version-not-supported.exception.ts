...
/**
 * 定义 *Http Version Not Supported* 类型错误的 HTTP 异常。
 *
...
export class HttpVersionNotSupportedException extends HttpException {
  /**
   * 创建一个 `HttpVersionNotSupportedException` 异常实例。
   *
   * @example
   ...
   * - `statusCode`：这将是值 505。
   * - `message`：默认为字符串 `'HTTP Version Not Supported'`；通过在 `objectOrError` 参数中提供字符串来覆盖。
   *
